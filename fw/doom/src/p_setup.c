...
extern const char * bakemaps[];
extern const int numvertexes_baked[];
extern const vertex_t*	vertexes_baked[];
extern const int		numnodes_baked[];
extern const node_t*		nodes_baked[];
...
// CNLohr Note: We can't really do much here, since this data /needs/ to be modified runtime.
...
void P_LoadBlockMap (int lump)
{
    ...
    blockmaplump = W_CacheLumpNum_Old (lump,PU_LEVEL);
...
void P_LoadVertexes (int lump)
{
    ...
    // Free buffer memory.
    //Z_Free (data);  XXX: CacheLumpNum points to our static array.
}
...
printf("LOADING FROM MAP: %s\n", lumpname );
    ...
    P_LoadSubsectors (lumpnum+ML_SSECTORS);
//   P_LoadNodes (lumpnum+ML_NODES);
	numnodes = numnodes_baked[mapid];
	nodes = nodes_baked[mapid];
    P_LoadSegs (lumpnum+ML_SEGS);
